...
- **Phase 6 — ship to production (Part A)** (2026-06-03): All code/config for the locked single-combined-service Railway topology, verified locally end-to-end. **Not committed/pushed** (awaiting user approval + Part B). Plan: `.claude/plans/ticklish-chasing-marble.md`.
  - **Topology:** one container, gunicorn → Django serves API/admin **and** the pre-built React SPA via whitenoise from the same origin (zero CORS); one Railway Postgres. Local `docker-compose` **converged** onto the same combined image (single `web` service + `db`); removed the now-dead `backend/Dockerfile`, `frontend/Dockerfile`, `frontend/nginx.conf`.
  - **New combined `Dockerfile`** (repo root, multi-stage): `node:20-alpine` builds Vite `dist/` → `python:3.12-slim` installs backend, copies `backend/` + `resources/` + the built SPA to `/app/frontend_build`, runs `entrypoint.sh` (migrate → collectstatic → seed_domains → import_questions → gunicorn on `$PORT`).
  - **settings.py hardening:** `DATABASE_URL` via `dj-database-url` (fallback to `DB_*`); `SECURE_PROXY_SSL_HEADER`; env-overridable `SESSION/CSRF_COOKIE_SECURE` (default `not DEBUG`); auto-add `RAILWAY_PUBLIC_DOMAIN` to `ALLOWED_HOSTS` + `CSRF_TRUSTED_ORIGINS`; `STORAGES['staticfiles'] = whitenoise CompressedStaticFilesStorage` (**not** Manifest — Vite already content-hashes; manifest would re-hash without rewriting `index.html` refs); `FRONTEND_BUILD_DIR` added to `STATICFILES_DIRS` when present.
  - **SPA serving:** `vite.config.js` `base` is `/static/` on build (`/` in dev); new `securityplus/views.py` adds unauthenticated `healthz` + a `spa_index` catch-all (`re_path(r'^(?!api/|admin/|static/).*$')`, last) returning the built `index.html`.
  - **Smoke tests** (replaced empty stubs): `questions/tests.py` (healthz 200 no-auth; questions API 403 anon) + `progress/tests.py` (csrf → login → session → next → submit correct, scored). All green.
  - **CI:** `.github/workflows/ci.yml` — `test` job (Postgres service, migrate, `manage.py test`, `npm ci`+`npm run build`) + `deploy` job (`needs: test`, push-to-main only, `railway up` via `RAILWAY_TOKEN`). `railway.json` pins the Dockerfile builder + healthcheck.
  - **Local verification (combined image):** `docker compose build web` + `up` → healthz 200; SPA root + `/dashboard` deep link 200 (text/html); `/static/assets/*.js` 200 via whitenoise; anon questions API 403; live register → session → submit returns the two-strike hint. `manage.py test` green.
  - **Remaining (Part B, user only):** Railway account/project/Postgres, `RAILWAY_TOKEN` secret, set service env vars, approve the commit + push, then `createsuperuser` + live smoke test.
...
- **✅ RESOLVED (2026-06-03) — `backend/requirements.txt` build blocker.** Was: a Windows `pip freeze` pinned **`pywin32==311`** (no Linux wheel → `docker compose build backend` failed) and omitted `gunicorn`/`whitenoise`. Fixed by guarding `pywin32==311; sys_platform == "win32"` and adding `gunicorn==26.0.0` + `whitenoise==6.12.0`; backend image now builds. ⚠️ **Do NOT regenerate this file with a bare `pip freeze` on Windows again** — it reintroduces both defects. ✅ Phase 6 added `dj-database-url==2.3.0` + the Railway `DATABASE_URL`/secure-cookie/static settings.
- **Combined image is the deploy + local target (Phase 6).** Prod and local both run the repo-root `Dockerfile` (one `web` service in `docker-compose.yml`). There is no separate backend/frontend image anymore. The backend is **not** volume-mounted, so backend code changes need `docker compose up -d --build web` to take effect (a bare restart runs the old image).
- **Whitenoise static storage is `CompressedStaticFilesStorage`, NOT the Manifest variant.** Vite content-hashes its own asset filenames; `ManifestStaticFilesStorage` would re-hash them without rewriting the references inside `index.html`, breaking the SPA. Do not "upgrade" it to manifest.
- **The local venv** (`venv/`) needed `dj-database-url` + `whitenoise` installed to run `manage.py` after Phase 6 (they were in `requirements.txt` but the venv predated them). The Docker image installs from `requirements.txt` so it's unaffected.
...
| Phase 6 (Part A) | Infra/deploy only (no content change) — Domain 5 (Governance/Risk/Compliance) + Domain 4 (Security Operations) applied to own infra: combined container, secure cookies + TLS proxy header, secret/`DATABASE_URL` management, CI gate, health checks. |
